...
one must call ``np.any(np.isnan(array))``. The
:meth:`bottleneck.anynan` function interleaves the :meth:`np.isnan`
check with :meth:`np.any` pre-exit, enabling up to an ``O(N)``
speedup relative to numpy.

Bottleneck strives to be a drop-in accelerator for NumPy functions.
When using the following libraries, Bottleneck support is
automatically enabled and utilized:
