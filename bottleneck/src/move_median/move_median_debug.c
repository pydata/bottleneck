...
int main(void) { return mm_unit_test(); }
...
int mm_unit_test(void) {
  ...
  sprintf(err_msg, "move_median failed with window=%d, min_count=%d", window,
          min_count);
...
void mm_print_chain(mm_handle *mm) {
  ...
  printf("\nchain\n");
  node = mm->oldest;
  printf("\t%6.2f region %d idx %lu addr %p\n", node->ai, node->region,
         node->idx, node);
...
void mm_dump(mm_handle *mm) {
  ...
  printf("\t%lu n_n\n", mm->n_n);
  printf("\t%lu min_count\n", mm->min_count);
  printf("\t%lu s_first_leaf\n", mm->s_first_leaf);
  printf("\t%lu l_first_leaf\n", mm->l_first_leaf);
