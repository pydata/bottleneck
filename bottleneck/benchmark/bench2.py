...
FUNCTIONS = ['nansum', 'nanmean', 'nanstd', 'nanvar', 'nanmin', 'nanmax']
...
def bench2(shapes=[(1000, 1000), (1000, 1000), (1000, 1000), (1000, 1000),
    ...
    print("    Speed is bn 1.2 time divided by bn 1.3 time")
    ...
    for test in suite:
        ...
        fmt = name + "%8.2f" + "%11.2f"*(len(shapes) - 1)
...
def benchsuite(shapes, dtype, nans, axes, order, functions):
    ...
    for func in funcs:
        ...
        setup = """
            from bottleneck import %s2 as bn_func
            from bottleneck import %s as sl_func
        """ % (func, func)
    ...
    for func in funcs:
        ...
        setup = """
            from bottleneck import %s2 as bn_func
            from bottleneck import %s as sw_func
            w = a.shape[axis] // 5
        """ % (func, func)
        run['setups'] = getsetups(setup, shapes, nans, axes, dtype, order)
        suite.append(run)
