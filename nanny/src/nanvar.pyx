...
def nanvar_selector(arr, axis):
    "Return nanvar function that matches `arr` and `axis` and return `arr`."
...
@cython.boundscheck(False)
@cython.wraparound(False)
def nanvar_2d_float64_axisNone(np.ndarray[np.float64_t, ndim=2] a, int ddof=0):
...
@cython.boundscheck(False)
@cython.wraparound(False)
def nanvar_3d_float64_axisNone(np.ndarray[np.float64_t, ndim=3] a, int ddof=0):
